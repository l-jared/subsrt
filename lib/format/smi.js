...
function parse(content, options) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    // <SYNC Start = 1000>
    var match = /^\<SYNC[^\>]+Start\s*=\s*["']?(\d+)["']?[^\>]*\>([\s\S]*)/gi.exec(
      part,
    );
    if (match) {
      var caption = {};
      ...
      var blank = true;
      var p = /^\<P[^\>]+Class\s*=\s*["']?([\w\d\-_]+)["']?[^\>]*\>([\s\S]*)/gi.exec(
        caption.content,
      );
      if (!p) {
...
function build(captions, options) {
  ...
  for (var i = 0; i < captions.length; i++) {
    ...
    if (typeof caption.type === 'undefined' || caption.type == 'caption') {
      ...
      content +=
        '  <P Class=LANG>' +
        helper.htmlEncode(caption.text || '') +
        (options.closeTags ? '</P>' : '') +
        eol;
      ...
      content +=
        '  <P Class=LANG>' + '&nbsp;' + (options.closeTags ? '</P>' : '') + eol;
      if (options.closeTags) {
        content += '</SYNC>' + eol;
      }
